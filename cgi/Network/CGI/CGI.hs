...
import Cookie (Cookie(..), showCookie, newCookie, findCookie)
import qualified Cookie as Cookie (deleteCookie)
import Accept
import Header
import Monad
import Protocol
...
getVar :: MonadCGI m =>
       ...
       -> m (Maybe String)
getVar name = liftM (Map.lookup name) $ cgiGet Monad.cgiVars
