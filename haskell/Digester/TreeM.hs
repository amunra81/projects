...
module TreeM (Tree,na,node,root,leaf,toPassParent,value,position,index,commonIndexes) where 
import Data.Monoid
...
children :: Monoid (m (Tree m a)) => Tree m a -> m (Tree m a)
children tree = case tree of
                    Node _ xs _ _   -> xs
                    Root _ xs       -> xs
                    Leaf _ _ _      -> mempty
